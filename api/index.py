...
from core.persistence import DiskPersistenceManager, get_persistence_manager
...
BOOK_PROJECTS_DIR = os.environ.get("BOOK_PROJECTS_DIR", "./book_projects")
...
def get_disk_persistence_manager():
    """Get or create the disk persistence manager."""
    global _persistence_manager
    if _persistence_manager is None:
        _persistence_manager = DiskPersistenceManager(BOOK_PROJECTS_DIR)
...
def get_orchestrator():
    ...
    if _orchestrator is None:
        _orchestrator = Orchestrator(
            ...
            persistence_manager=get_disk_persistence_manager(),
            auto_save=True
        )
...
# =============================================================================
# Persistence & Version History Endpoints
# =============================================================================

@app.get("/api/system/saved-projects")
async def list_saved_projects(auth: bool = Depends(require_auth)):
    """List all projects saved to disk."""
    pm = get_disk_persistence_manager()
    return {"projects": pm.list_saved_projects()}


@app.get("/api/projects/{project_id}/chapters/{chapter_number}/versions")
async def get_chapter_versions(
    ...
    pm = get_disk_persistence_manager()
    versions = pm.get_chapter_versions(project_id, chapter_number)
    return {
...
@app.post("/api/projects/{project_id}/chapters/{chapter_number}/restore")
async def restore_chapter_version(
    ...
    pm = get_disk_persistence_manager()
    success = pm.restore_chapter_version(project, chapter_number, request.backup_filename)
    ...
    return {
        "success": True,
        "message": f"Chapter {chapter_number} restored from {request.backup_filename}"
    }
...
@app.post("/api/projects/{project_id}/check-duplicate")
async def check_duplicate_content(
    project_id: str,
    request: CheckDuplicateRequest,
    auth: bool = Depends(require_auth)
):
    """Check if content is duplicate of an existing chapter."""
    pm = get_disk_persistence_manager()
    duplicate_of = pm.check_duplicate_content(project_id, request.content)

    return {
        "is_duplicate": duplicate_of is not None,
        "duplicate_of_chapter": duplicate_of,
        "message": f"Content matches chapter {duplicate_of}" if duplicate_of else "No duplicate found"
    }


@app.post("/api/projects/{project_id}/save")
async def save_project_to_disk(project_id: str, auth: bool = Depends(require_auth)):
    """Manually save project state to disk."""
    project = get_orchestrator().get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")

    pm = get_disk_persistence_manager()
    pm.save_project_state(project)
