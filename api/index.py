...
class CoverGenerateRequest(BaseModel):
    """Request model for cover generation."""
    cover_type: str = "front"  # front, back, spine
    style: str = "professional cinematic book cover"


class ChapterIllustrationRequest(BaseModel):
    """Request model for chapter illustration."""
    chapter_number: int
    style: str = "moody atmospheric illustration"


@app.get("/api/system/image-status")
async def image_gen_status(auth: bool = Depends(require_auth)):
...
async def generate_cover(
    ...
    try:
        ...
        if result["success"]:
            ...
            if "images" not in project.manuscript:
                project.manuscript["images"] = {}

            project.manuscript["images"][f"cover_{request.cover_type}"] = {
                "type": f"cover_{request.cover_type}",
...
@app.post("/api/projects/{project_id}/generate-chapter-image/{chapter_number}")
    ...
    if not chapter:
        ...
        for layer in project.layers.values():
            if "chapter_blueprint" in layer.agents:
                agent_state = layer.agents["chapter_blueprint"]
                if agent_state.current_output:
                    blueprint = agent_state.current_output.content
                    break

        if blueprint:
            for ch in blueprint.get("chapter_outline", []):
                if ch.get("number") == chapter_number:
                    chapter = {
                        "number": chapter_number,
...
async def list_project_images(project_id: str, auth: bool = Depends(require_auth)):
    """List all generated images for a project."""
    project = get_orchestrator().get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")

    images = project.manuscript.get("images", {})

    # Return metadata without the full base64 data
    image_list = []
    for key, img in images.items():
        image_list.append({
            "key": key,
            "type": img.get("type"),
            "chapter_number": img.get("chapter_number"),
            "style": img.get("style"),
            "has_image": bool(img.get("image_base64"))
        })

    return {"images": image_list}


@app.get("/api/projects/{project_id}/images/{image_key}")
async def get_project_image(project_id: str, image_key: str, auth: bool = Depends(require_auth)):
...
@app.delete("/api/projects/{project_id}/images/{image_key}")
async def delete_project_image(project_id: str, image_key: str, auth: bool = Depends(require_auth)):
    ...
    del project.manuscript["images"][image_key]

    return {"success": True, "message": f"Image {image_key} deleted"}
