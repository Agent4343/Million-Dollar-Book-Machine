...
class TestGatherInputsPerformance(unittest.TestCase):
    ...
    def test_gather_inputs_finds_output_by_key(self):
        ...
        # Pick a downstream agent whose inputs list 'some_custom_output_key'
        # We'll just call gather_inputs on any agent and check the key appears if declared
        # For simplicity: inject the key into the test agent's required inputs via monkey-patch
